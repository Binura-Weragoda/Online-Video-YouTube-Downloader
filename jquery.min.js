...
function k_parseDuration(i){if(!i) return '';var p=[],l=0,r=i.match(/(?:(\d*)Y)?(?:(\d*)M)?(?:(\d*)W)?(?:(\d*)D)?T(?:(\d*)H)?(?:(\d*)M)?(?:(\d*)S)?/i),e=[{pos:1,multiplier:31536e3},{pos:2,multiplier:2592e3},{pos:3,multiplier:604800},{pos:4,multiplier:86400},{pos:5,multiplier:3600},{pos:6,multiplier:60},{pos:7,multiplier:1}];if(null===r)return"";for(var s=0;s<e.length;s++)void 0!==r[e[s].pos]&&(l+=parseInt(r[e[s].pos])*e[s].multiplier);return 3599<l&&(p.push(parseInt(l/3600)),l%=3600),p.push(("0"+parseInt(l/60)).slice(-2)),p.push(("0"+l%60).slice(-2)),p.join(":")};
...
function renderFail(fmess) {
    ...
    var fm_html = '<div class="error"><p>'+fmess+'</p></div>';
    $("#search-result").html(fm_html);
};
function renderListVideo(vitems) {
    if(jQuery.isEmptyObject(vitems))
    {
        renderFail("Invalid Youtube video url.")
    }
    else
    {
...
function convertSuccess(dlink) {
    $("#mesg-convert").addClass('hidden');
    $("#breload").addClass('hidden');
    $("#asuccess").attr('href', dlink).removeClass('hidden');
    $("#cnext").removeClass('hidden');
};
function convertFailed(mess) {
    renderFail(mess);
};
function checkTask(b_id) {
    $.ajax({
        type: "POST",
        ...
        success: function(rdata, textStatus, xhr){
            if(xhr.status != 200){
                checkTask(b_id);
            }else if(rdata.c_status == 'CONVERTED'){
                convertSuccess(rdata.dlink);
            }else if(rdata.c_status == 'FAILED'){
                convertFailed(rdata.mess);
            }
            else {
...
function convertFile(kinfo) {
    if(kinfo==0){
        kinfo = $('#formatSelect').find(":selected").val();
    }
    $.ajax({
        type: "POST",
        url: k_url_convert,
        ...
        beforeSend : function(){
            ...
            if(typeof k_is_mini!='undefined' && k_is_mini){
                $("#breload").removeClass('hidden');
            }
        },
        success: function(rdata){
            if(typeof rdata.c_status == 'undefined' ){
                return convertFailed(typeof rdata.mess == 'undefined'?'':rdata.mess)
            }
            else if(rdata.c_status == 'CONVERTING' && typeof rdata.b_id!='undefined' && typeof rdata.e_time!='undefined')
            {
                window.setTimeout(function() { checkTask(rdata.b_id);  }, 5000);
            }
            else if(rdata.c_status == 'CONVERTED' && typeof rdata.dlink!='undefined')
            {
                convertSuccess(rdata.dlink);
            } else {
                return convertFailed(typeof rdata.mess == 'undefined'?'':rdata.mess)
        ...
        error: function(data){
            window.setTimeout(function() { convertFile(kinfo); }, 5000);
        }
    });
};
...
function renderDetail(vData) {
    ...
    detail_html += '<span id="mesg-convert" class="form-control mesg-convert hidden"> <span class="lds-dual-ring"></span> '+ txt_processing +'</span>' +
        ...
        '<a id="cnext" class="form-control mesg-convert hidden" href="'+k_url_next+'">Convert next</a>' +
        '<button id="breload" class="form-control btn-blue-small hidden" rel="nofollow" onclick="window.location.reload();"> Get link </button> &nbsp &nbsp' +
        '</div><br/><br/><br/> ' +
...
function renderFacebook(vData) {
    ...
    var detail_html = '<div class="detail"><div class="thumbnail">\n' +
        '<img src="'+vData.thumbnail+'">\n' +
        '<div class="content"><div class="clearfix">\n' +
        '<h3>'+KHtmlEncode(vData.title)+'</h3>\n' +
        '<p class="mag0">'+k_parseDuration(vData.duration)+'</p>';
...
function load_qc() {
    ...
    $.getScript( "https://yt1s.com/statics/js/pn.js?v=1.4" );
...
function parse_query_string(query) {
    var q_params = query.split("&");
    var query_string = {};

    if(!jQuery.isEmptyObject(q_params))
    {
        $.each(q_params, function(i,q_param) {
            var pair = q_param[i].split("=");
...
$(document).ready(function () {
    ...
    if(/[?&]q=/.test(window.location.search)) {
        var k_query = window.location.search.substring(1);
        var qs = parse_query_string(k_query);
        if (!jQuery.isEmptyObject(qs) && qs.q !== undefined && qs.q !== null) {
            ksearchvideo();
        }
    }
